...
namespace TokenClient.Common.Tokens
{
    ...
    public class SwtSecurityTokenHandler : SecurityTokenHandler
    {
        ...
        /// </summary> 
        public SwtSecurityTokenHandler()
        {
        }
        ...
        public override Type TokenType
        {
            get
            {
                return typeof(SwtSecurityToken);
            }
        }
        ...
        public override bool CanReadToken(XmlReader reader)
        {
            ...
            if (reader != null)
            {
                if (reader.IsStartElement(BinarySecurityToken)
                    && (reader.GetAttribute(ValueType) == SwtSecurityTokenConstants.ValueTypeUri))
                {
                    canRead = true;
                }
            }
        ...
        public override SecurityToken CreateToken(SecurityTokenDescriptor tokenDescriptor)
        {
            ...
            properties.Add(SwtSecurityTokenConstants.Id, Guid.NewGuid().ToString());
            properties.Add(SwtSecurityTokenConstants.Issuer, tokenDescriptor.TokenIssuerName);
            properties.Add(SwtSecurityTokenConstants.Audience, tokenDescriptor.AppliesToAddress);
            properties.Add(SwtSecurityTokenConstants.ExpiresOn, SecondsFromSwtBaseTime(tokenDescriptor.Lifetime.Expires));
            properties.Add(SwtSecurityTokenConstants.ValidFrom, SecondsFromSwtBaseTime(tokenDescriptor.Lifetime.Created));
            ...
            SwtSecurityToken token = new SwtSecurityToken(properties);
            return token;
        }

        private string SecondsFromSwtBaseTime(DateTime? time)
        {
            DateTime date = time ?? SwtSecurityToken.SwtBaseTime;
            TimeSpan t1 = date.Subtract(SwtSecurityToken.SwtBaseTime);
        ...
        protected SecurityToken ReadSecurityTokenFromString(string serializedToken)
        {
            ...
            SwtSecurityToken swt = new SwtSecurityToken(properties, serializedToken);
        ...
        protected virtual bool IsReservedKeyName(string keyName)
        {
            ...
            switch (keyName)
            {
                case SwtSecurityTokenConstants.Audience:
                case SwtSecurityTokenConstants.ExpiresOn:
                case SwtSecurityTokenConstants.Id:
                case SwtSecurityTokenConstants.Issuer:
                case SwtSecurityTokenConstants.ValidFrom:
                case SwtSecurityTokenConstants.Signature: return true;
